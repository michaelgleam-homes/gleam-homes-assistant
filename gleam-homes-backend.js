...
app.post('/webhook/message', async (req, res) => {
  try {
    ...
    console.log(`\n📨 SMOOBU WEBHOOK empfangen:`);
    console.log(`Action: ${action}`);
    console.log(`Data:`, JSON.stringify(data));

    // Smoobu sendet newMessage Webhooks
    if (action !== 'newMessage') {
      console.log(`⚠️  Webhook ignoriert (nicht newMessage): ${action}`);
      return res.json({ success: false, reason: 'Not a newMessage webhook' });
    }

    // Nur Gäste-Nachrichten verarbeiten (sender === "guest")
    if (data?.sender !== 'guest') {
      console.log(`⚠️  Webhook ignoriert (nicht von Gast): ${data?.sender}`);
      return res.json({ success: false, reason: 'Not a guest message' });
    }

    // Extrahiere die Daten aus Smoobu-Format
    const messageId = data?.id;
    const bookingId = data?.booking?.id;
    const messageText = data?.message || data?.text || '';
    
    // Gast-Info aus Smoobu abrufen (später - jetzt simplified)
    const guestName = data?.guest_name || 'Gast';
    const guestId = bookingId;

    if (!guestId || !messageText) {
      console.log(`❌ Erforderliche Felder fehlen: guestId=${guestId}, message=${messageText}`);
      return res.status(400).json({ error: 'Missing required fields' });
    }

    console.log(`\n✅ Verarbeite Nachricht von ${guestName} (Booking: ${guestId}): "${messageText}"`);

    const result = await processGuestMessage(guestName, messageText, guestId);
    const smoobuResult = await sendToSmoobu(guestId, result.message);

    console.log(`✅ Antwort gesendet: ${result.type}`);
